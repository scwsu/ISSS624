...
```{r}
draw_honeycomb_map_fixed <- function(input_data) {
...
```{r}
draw_honeycomb_map_quantile <- function(input_data) {
  map_output <- tm_shape(input_data) +
    tm_fill(
      col = "TOT_TRIPS",
      palette = "Reds",
...
draw_honeycomb_map_quantile(honeycombgrid_wdmp)
```
...
draw_honeycomb_map_quantile(honeycombgrid_wdap)
```
...
draw_honeycomb_map_quantile(honeycombgrid_wehmp)
```
