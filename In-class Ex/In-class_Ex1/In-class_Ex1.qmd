...
#### **3.5. Importing Attribute Data into R**
...
```{r}
duplicate <- origin_SZ %>%   
  group_by_all() %>%   filter(n()>1) %>%   ungroup()
```

```{r}
origintrip_SZ <- left_join(mpsz,                             origin_SZ,                            by = c("SUBZONE_C" = "ORIGIN_SZ"))
```
